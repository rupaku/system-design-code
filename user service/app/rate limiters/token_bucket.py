...
RATE_LIMIT_CAPACITY = 10  # Max tokens in bucket (Burst size)
REFILL_RATE = 1.0         # Tokens per second (Steady rate)
...
# Dependency to implement Token Bucket rate limiting
async def token_bucket_limiter(request: Request):
    ...
    # Note: Using the 'redis' object defined in your global scope
    ...
    if not bucket:
        ...
        last_tokens = float(bucket["tokens"])
        last_refill = float(bucket["last_refill"])
        
        # 3. Calculate refill based on time passed
        time_passed = now - last_refill
        refilled_tokens = last_tokens + (time_passed * REFILL_RATE)
        tokens = min(float(RATE_LIMIT_CAPACITY), refilled_tokens)
        last_refill = now
    ...
    if tokens >= 1:
        ...
        # Cleanup: Expire key after 1 hour of inactivity
...
# Use this Annotated dependency to protect routes
RateLimitDep = Annotated[bool, Depends(token_bucket_limiter)]

# --- API Setup & Routes ---
app = FastAPI(lifespan=lifespan)
router = APIRouter(prefix="/api/v1", dependencies=[Depends(token_bucket_limiter)])
