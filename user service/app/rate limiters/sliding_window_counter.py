...
WINDOW_SIZE = 60  # Window duration in seconds (1 minute)
MAX_REQUESTS = 10 # Max requests allowed per window
...
async def sliding_window_counter_limiter(request: Request):
    # 1. Identify client (IP address)
    user_ip = request.client.host if request.client else "127.0.0.1"
    
    now = time.time()
    
    # 2. Identify the current and previous window timestamps
    current_window_start = int(now // WINDOW_SIZE) * WINDOW_SIZE
    prev_window_start = current_window_start - WINDOW_SIZE
    
    curr_key = f"swc:{user_ip}:{current_window_start}"
    prev_key = f"swc:{user_ip}:{prev_window_start}"
    
    # 3. Get counts for both windows in a single pipeline
    ...
    # 4. Calculate the weight of the previous window
    # Example: If we are 20% into the current window, we take 80% of the previous window
    time_passed_in_curr_window = now - current_window_start
    weight = 1 - (time_passed_in_curr_window / WINDOW_SIZE)
    
    # 5. Calculate the "Sliding" count
    sliding_count = (prev_count * weight) + curr_count
    
    if sliding_count >= MAX_REQUESTS:
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail=f"Rate limit exceeded. Sliding count: {int(sliding_count)}"
        )
    
    # 6. If allowed, increment the current window counter
    await redis.incr(curr_key)
    if curr_count == 0:
        # Set expiration to clean up (2 windows worth of time is enough)
        await redis.expire(curr_key, WINDOW_SIZE * 2)
    
    return True

# Type Alias
SlidingWindowDep = Annotated[bool, Depends(sliding_window_counter_limiter)]

# --- API Setup & Routes ---
app = FastAPI(lifespan=lifespan)
router = APIRouter(prefix="/api/v1", dependencies=[Depends(sliding_window_counter_limiter)])
