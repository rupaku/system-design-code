...
async def fixed_window_limiter(request: Request):
    # 1. Identify client (IP address)
    user_ip = request.client.host if request.client else "127.0.0.1"
    
    # 2. Generate a unique key for the current window
    # By dividing time by WINDOW_SIZE, we get the same 'bucket' ID for the whole minute
    current_window = int(time.time() // WINDOW_SIZE)
    key = f"fixed_window:{user_ip}:{current_window}"
    
    # 3. Increment the counter in Redis atomically
    # 'incr' returns the value after incrementing
    current_count = await redis.incr(key)
    
    # 4. If it's the first request in this window, set expiration
    if current_count == 1:
    ...
    if current_count > MAX_REQUESTS:
        raise HTTPException(
            ...
            detail=f"Rate limit exceeded. Maximum {MAX_REQUESTS} requests per {WINDOW_SIZE} seconds allowed."
        )
    
    return True

# Type Alias for route dependency
FixedWindowDep = Annotated[bool, Depends(fixed_window_limiter)]

# --- API Setup & Routes ---
app = FastAPI(lifespan=lifespan)
router = APIRouter(prefix="/api/v1", dependencies=[Depends(fixed_window_limiter)])
