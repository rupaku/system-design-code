...
BUCKET_CAPACITY = 10  # Maximum number of requests the bucket can hold
LEAK_RATE = 1.0       # Requests processed per second (1.0 = 1 req/sec)
...
async def leaky_bucket_limiter(request: Request):
    # 1. Identify client (IP address)
    user_ip = request.client.host if request.client else "127.0.0.1"
    key = f"leaky_bucket:{user_ip}"
    
    now = time.time()
    
    # 2. Get current state from Redis
    # We track 'level' (current water) and 'last_leak' (timestamp)
    ...
    if not bucket:
        ...
        last_level = float(bucket["level"])
        last_leak_time = float(bucket["last_leak"])
        
        # 3. Calculate how much has "leaked" out since the last request
        time_passed = now - last_leak_time
        leaked_amount = time_passed * LEAK_RATE
        
        # New level = current level - leaked amount + 1 (for this new request)
        # Level cannot drop below 0
        level = max(0.0, last_level - leaked_amount) + 1.0
        last_leak = now

    # 4. Check for Overflow
    if level <= BUCKET_CAPACITY:
        # Bucket has room: Save updated level
        await redis.hset(key, mapping={
            "level": str(level),
            "last_leak": str(last_leak)
        })
        await redis.expire(key, 600) # Cleanup after 10 mins of inactivity
        return True
    else:
        # Bucket is full: Overflow!
        raise HTTPException(
            status_code=status.HTTP_429_TOO_MANY_REQUESTS,
            detail="Server is busy. The request queue is full."
        )

# Type Alias
LeakyLimitDep = Annotated[bool, Depends(leaky_bucket_limiter)]
  


# --- API Setup & Routes ---
app = FastAPI(lifespan=lifespan)
router = APIRouter(prefix="/api/v1", dependencies=[Depends(leaky_bucket_limiter)])
