...
MONGO_URI_LIST = os.getenv("MONGO_URI", "mongodb://localhost:27017/user_db").split(",") if "," in os.getenv("MONGO_URI", "") else [os.getenv("MONGO_URI", "mongodb://localhost:27017/user_db")]
...
async def lifespan(app : FastAPI):
    ...
    #--- MongoDB Setup with Round Robin Pool ---
    mongo_clients = [AsyncIOMotorClient(uri) for uri in MONGO_URI_LIST]

    # 2. Create the iterator for Round Robin selection
    # cycle() returns an iterator that yields elements from the iterable 
    # and saves a copy of each element. When the iterable is exhausted, 
    # it restarts from the saved copy.
    app.state.db_clients = mongo_clients
    app.state.client_cycler = itertools.cycle(mongo_clients)
    #mongo_client = mongo_clients[0]
...
async def get_db_collection(request: Request) -> AsyncIOMotorCollection:
    _cycler = getattr(request.app.state, "client_cycler", None)
    if _cycler is None:
        raise HTTPException(status_code=500, detail="db connection error")
    next_client = next(_cycler)
...
if __name__ == "__main__":
    ...
    print("Connect to MongoDB at:", MONGO_URI_LIST)
    print("Access docs at: http://127.0.0.1:8000/docs")
    uvicorn.run("main_round_robin:app", host="127.0.0.1", port=8000, reload=True)