...
# IP Hash Load Balancing Configuration
# Configure multiple MongoDB instances for IP-based routing
...
async def lifespan(app : FastAPI):
    ...
    #--- MongoDB Setup with IP Hash Pool ---
    mongo_clients = [AsyncIOMotorClient(uri) for uri in MONGO_URI_LIST]

    # Store as a fixed list for IP hash-based selection
    app.state.db_clients = mongo_clients
    
    print(f"✅ Initialized {len(mongo_clients)} MongoDB clients for IP hash load balancing")
...
# Dependency to get the MongoDB collection using IP hash-based selection
async def get_db_collection(request: Request) -> AsyncIOMotorCollection:
    ...
    # 1. Get the Client IP
    # Priority: X-Forwarded-For > X-Real-IP > client.host > fallback
    client_ip = (
    ...
    # 2. Create a deterministic Hash of the IP
    # Using SHA256 for better distribution than MD5
    hash_digest = hashlib.sha256(client_ip.encode()).hexdigest()
    hash_int = int(hash_digest, 16)

    # 3. Map the hash to an index (Modulo)
    # This ensures the same IP always goes to the same MongoDB instance
    server_index = hash_int % len(clients)
    
    # Log for debugging (remove in production)
    print(f"🔍 IP: {client_ip} -> Server Index: {server_index} (of {len(clients)} servers)")
    
    selected_client = clients[server_index]
    return selected_client[DATABASE_NAME][COLLECTION_NAME]
...
@router.get("/")
def say_hello():
    return "Hello world from IP Hash Load Balancer!"

@router.get("/ip-info")
async def get_ip_info(request: Request):
    """Get information about client IP and which MongoDB server it maps to"""
    try:
        clients = getattr(request.app.state, "db_clients", [])
        
        # Get client IP using same logic as get_db_collection
        client_ip = (
            request.headers.get("x-forwarded-for", "").split(",")[0].strip() or
            request.headers.get("x-real-ip") or
            (request.client.host if request.client else "127.0.0.1")
        ...
        # Calculate hash and server index
        hash_digest = hashlib.sha256(client_ip.encode()).hexdigest()
        hash_int = int(hash_digest, 16)
        server_index = hash_int % len(clients) if clients else 0
        
        return {
            "client_ip": client_ip,
            "hash_digest": hash_digest[:16] + "...",  # Show first 16 chars
            "server_index": server_index,
            "total_servers": len(clients),
            "mongodb_uris": MONGO_URI_LIST,
            "load_balancing_method": "IP Hash"
        }
...
if __name__ == "__main__":
    print("--- Starting User Service API with IP Hash Load Balancing ---")
    print("MongoDB URIs:", MONGO_URI_LIST)
    print("Load Balancing Method: IP Hash (consistent client-to-server mapping)")
    print("Access docs at: http://127.0.0.1:8000/docs")
    uvicorn.run("main_ip_hash:app", host="127.0.0.1", port=8000, reload=True)