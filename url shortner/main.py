import random
import string
from fastapi import FastAPI, Body, APIRouter, HTTPException , status , Depends, Request
...
async def create_short_url(
    ...
    links_collection : CollectionDep
    ):
    short_code : str
    ...
    if not final_document:
        raise HTTPException(
            status_code=500, detail="Failed to create short URL (not found after insert)"
        )
...
async def handle_redirect(short_code: str, links_collection: CollectionDep):
    # Find the link
    ...
    # Ensure the URL is a plain string and has http/https
    target_url = str(link_in_db.long_url)
...
async def get_long_url(
    ...
    return ShortenResponse(
        unique_id = link_in_db.short_code,
...
async def expire_long_url(short_code: str,
    ...
    if not updated_document:
        raise HTTPException(
            status_code=500, detail="Failed to load updated document from DB"
        )
    ...
    return ShortenResponse(
        ...
        short_url = f"{BASE_DOMAIN}/{updated_link.short_code}",
